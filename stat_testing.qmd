...
# Paso 1: contar diseñadores únicos por país
df_pais <- df_award %>%
  group_by(designer_country) %>%
  summarise(diseñadores_unicos = n_distinct(designer_name), .groups = "drop") %>%
  arrange(desc(diseñadores_unicos))

# Paso 2: graficar
p <- ggplot(df_pais, aes(x = reorder(designer_country, -diseñadores_unicos), y = diseñadores_unicos)) +
  coord_flip() +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Diseñadores únicos por país",
       x = "País",
       y = "Cantidad de diseñadores distintos") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

ggsave("grafico_disenadores.png", plot = p, width = 6, height = 14, dpi = 300)
```
